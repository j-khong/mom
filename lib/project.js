    ...
    */
    ...
    File.replaceInFile("NamespaceToReplace", namespace,
      [Path.join(imports, "common", "_base.js"),
        Path.join(imports, "client", "_base.js"),
...
function checkAndCleanSchemaValues(schemaValues) {
  ...
  schemaValues.forEach((fieldDef) => {
    ...
    if( null != val && val == "unique" ) { res.uniqueKeyFields.push(name); }
...
function generateSchema(modelFile, schema) {
  ...
  schema.fields.forEach((fieldDef) => {
    strSchema += "  " + fieldDef.name + ": {type: " + fieldDef.type + "},\n";
    strMethods += "  get" + fieldDef.name.capitalize() + "() { return this.data." + fieldDef.name + "; }\n";
  });
