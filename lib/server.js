...
module.exports = {
  test2: function (cliOptions) {
    var env = cliOptions["env"];
    var envfile = Conv.getSettingsFile(env);
    var serverConf = Settings.getServerConf(env);
    var upAppConfFile = Path.resolve(".", "env.list");
    createAppConfFile(upAppConfFile, envfile, serverConf);
  },
  deploy: function (cliOptions) {

    const env = cliOptions["env"];
    const envfile = Conv.getSettingsFile(env);
    if (!FS.existsSync(envfile)) { throw new ConfError("conf file " + envfile + " is missing"); }

    var prjConf = Settings.getProjectConf();
    var serverConf = Settings.getServerConf(env);
    if (!serverConf) {
      const aHeader = ["you have not configured your server for env " + env + ", let's do that!"];
      console.log(Log.formatHeader(aHeader));
      console.log("");
      const ip = ReadlineSync.question("what is the server ip? : ");
      const user = ReadlineSync.question("what is the server user? : ");
      const tld = ReadlineSync.question("what is the top level domain name? : ");
      const port = ReadlineSync.question("which port to expose? : ");
...
function createAppConfFile(pkgAppConfFile, envfile, serverConf) {
  // inline file content
  const c = FS.readFileSync(envfile, "utf8");
  const arr = c.split("\n").map((v) => { return v.trim(); });
  const inline = arr.join("");

  let url = `http://${serverConf.tld}`;
  if (serverConf.port) {
    url += ':' + serverConf.port;
  }
  const content = `
