...
Template.Login.events({
    ...
    'submit form#connection-form': function(event, template){
        ...
        if( emailVar === "" )
        { template.error.set("veuillez renseigner un email"); return; }

        if( passwordVar === "" )
        { template.error.set("veuillez renseigner un mot de passe"); return; }
    ...
    'submit form#create-account-form': function(event, template){
        ...
        { template.error.set("Les deux mot de passe saisis sont différents"); return; }
    ...
    'submit form#create-account-msg-form': function(event, template){
        ...
        Meteor.loginWithPassword(template.email, template.pwd, error2 => {
            if( error2 )
            { template.error.set("problème de connexion"); }
        });
    },
    ...
    'submit form#chg-pwd-form': function(event, template){
        ...
        if( emailVar === "" )
        { template.pwderror.set("veuillez renseigner un email"); return; }
